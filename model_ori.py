...
# from modeling_llama_kv import LlamaForCausalLM, LlamaRMSNorm as KVForCausalLM, RMSNorm
from modeling_mistral_kv import MistralForCausalLM, MistralRMSNorm as KVForCausalLM, RMSNorm
from utils import *
...
class MHCModel(nn.Module):
    ...
    def forward(
        ...
        if inference_mode:
            # 获取base model的top-k预测结果
            base_max_k = max(path[0] if path else 0 for path in node_paths) + 1
            # 只对序列的最后一个位置进行top-k采样
            last_token_logits = current_logits[:, -1:]  # [batch_size, 1, vocab_size]
            base_topk = last_token_logits.topk(k=base_max_k, dim=-1)
            ...
            for path in sorted_paths:
                ...
                for prefix_len in range(len(path), -1, -1):
                    ...
                    if prefix in cache_results:
                        current_hidden, current_logits_for_path = cache_results[prefix]
                        start_idx = prefix_len
                        break
                else:
                    start_idx = 0
                    current_hidden = prev_hidden
                    current_logits_for_path = current_logits
                
                # 从最长前缀之后开始计算
                for head_idx in range(start_idx, len(path)):
                    choice_idx = path[head_idx]
                    
                    if head_idx == 0:
                        # 第一个头从base model采样
                        pred_token = torch.cat([
                            input_ids[:, 1:],  # 去掉序列的第一个token
                            base_topk.indices[:, :, choice_idx]  # 将采样结果添加到末尾
                        ], dim=1)
                    else:
                        # 对于后续MHC头的结果，同样去掉第一个token，将采样结果添加到末尾
                        prev_path = tuple(path[:head_idx])
                        prev_logits = cache_results[prev_path][1]  # 获取缓存的logits
                        last_token_logits = prev_logits[:, -1:]  # 只取最后一个位置
                        k = choice_idx + 1
                        last_token_topk = last_token_logits.topk(k=k, dim=-1)
                        pred_token = torch.cat([
                            pred_token[:, 1:],  # 去掉序列的第一个token
                            last_token_topk.indices[:, :, -1]  # 将采样结果添加到末尾
                        ], dim=1)
                    
                    token_embed = self.base_model.model.embed_tokens(pred_token)
                    
                    # 应用RMSNorm并处理
                    norm_hidden = hidden_rmsnorm(current_hidden)
                    ...
                    current_logits_for_path = self.base_model.lm_head(mhc_hidden_states)
                    
                    # 缓存当前路径的结果
                    current_path = tuple(path[:head_idx + 1])
                    cache_results[current_path] = (mhc_hidden_states, current_logits_for_path)
                    current_hidden = mhc_hidden_states
                
                mhc_logits.append(current_logits_for_path)
        else:
            # 训练模式
    ...
    def mhc_generate(
        ...
        if hasattr(self, "mhc_choices") and self.mhc_choices == mhc_choices:
            # 加载缓存的MHC buffer
            mhc_buffers = self.mhc_buffers
        else:
            # 初始化MHC buffer
            mhc_buffers = generate_mhc_buffers(
                mhc_choices, device=self.base_model.device
            )
        self.mhc_buffers = mhc_buffers
        self.mhc_choices = mhc_choices
        ...
        mhc_logits, logits = initialize_mhc(  # 改为initialize_mhc
        ...
        for idx in range(max_steps):
            ...
            candidates, tree_candidates = generate_candidates(
                mhc_logits,  # 改为mhc_logits
                logits,
                mhc_buffers["tree_indices"],
                mhc_buffers["retrieve_indices"],
            )

            # 使用树形注意力验证候选项并获取预测
            mhc_logits, logits, outputs = tree_decoding(  # 保留函数名，但传入mhc相关参数
                self,
                tree_candidates,
                past_key_values,
                mhc_buffers["medusa_position_ids"],
                input_ids,
                mhc_buffers["retrieve_indices"],
            )
            ...
            # 更新input_ids和logits
            input_ids, logits, mhc_logits, new_token = update_inference_inputs(
                input_ids,
                candidates,
                best_candidate,
                accept_length,
